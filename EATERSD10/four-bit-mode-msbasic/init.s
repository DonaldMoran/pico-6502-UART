...
.else
  ...
  .endif
  .ifdef EATER
    jsr LCDINIT  ; Get LCD working in 4 bit mode first, need porta for RTS
  .endif
.endif
...
.ifndef KBD
  ...
  .endif
        ...
        stx     TEMPPT
.ifndef CONFIG_CBM_ALL
        lda     #<QT_MEMORY_SIZE  ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        ldy     #>QT_MEMORY_SIZE  ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        jsr     STROUT            
  .ifdef APPLE                     
        jsr     INLINX             
  .else                            
        jsr     NXIN              ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
  .endif                           
        stx     TXTPTR            ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        sty     TXTPTR+1          ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        jsr     CHRGET            ;DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
  .ifndef AIM65
    .ifndef SYM1
        cmp     #$41
        beq     PR_WRITTEN_BY
...
.ifdef CONFIG_2
        ...
        lda LINNUM+1
        cmp #$50       ; high byte of RAMTOP
        bcc @continue  ; if LINNUM < $5000, proceed
        lda LINNUM
        cmp #$00
        bcc @continue
        jmp L40FA      ; bail out early

@continue:
        lda     #$55 ; 01010101 / 10101010   ; DONALD AVOID U WHEN THIS HITS VIDEO RAM
.else
        lda     #$92 ; 10010010 / 00100100
.endif
        sta     (LINNUM),y                  ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        cmp     (LINNUM),y                  ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK  
        bne     L40FA                       ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        asl     a                           ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        sta     (LINNUM),y                  ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
        cmp     (LINNUM),y                  ; DONALD MORAN COMMENT OUT TO SKIP MEMORY CHECK
.ifdef CONFIG_CBM_ALL
        beq     L40D7
...
.endif
L40FA:
        lda     LINNUM                     ; DONALD COMMENT OUT
        ldy     LINNUM+1                   ; DONALD COMMENT OUT
;        lda     #$00                      ; DONALD MORAN JUST STATICLY SET IT TO SKIP MEMORY CHECK 
;        ldy     #$50                      ; DONALD MORAN JUST STATICLY SET IT
        sta     MEMSIZ
        sty     MEMSIZ+1
...
.ifndef CONFIG_CBM_ALL
  .ifdef APPLE
        ...
        jsr     NXIN
  .else
        lda     #<QT_TERMINAL_WIDTH  ; DONALD MORAN SKIP COMMENT OUT
        ldy     #>QT_TERMINAL_WIDTH  ; DONALD MORAN SKIP COMMENT OUT
        jsr     STROUT               ; DONALD MORAN SKIP COMMENT OUT
        jsr     NXIN                 ; DONALD MORAN SKIP COMMENT OUT
  .endif
        stx     TXTPTR               ; DONALD MORAN SKIP COMMENT OUT
        sty     TXTPTR+1             ; DONALD MORAN SKIP COMMENT OUT
        jsr     CHRGET               ; DONALD MORAN SKIP COMMENT OUT
        tay                          ; DONALD MORAN SKIP COMMENT OUT
        beq     L4136                ; DONALD MORAN SKIP COMMENT OUT
        jsr     LINGET
        lda     LINNUM+1
        bne     L4106
        lda     LINNUM
...
.endif
        ...
        lda     MEMSIZ+1
        sbc     TXTTAB+1
        jsr     LINPRT           ;DONALD MORAN SKIP COMMENT OUT
        lda     #<QT_BYTES_FREE  ;DONALD MORAN SKIP COMMENT OUT
        ldy     #>QT_BYTES_FREE  ;DONALD MORAN SKIP COMMENT OUT
        jsr     STROUT           ;DONALD MORAN SKIP COMMENT OUT
