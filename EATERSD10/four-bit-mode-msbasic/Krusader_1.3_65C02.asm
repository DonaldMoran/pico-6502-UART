...
BS	=$08		; backspace
SP	=$20		; space
;CR	=$0D		; carriage return  *** Already defined in msbasic
;LF	=$0A		; line feed *** Already defined in msbasic
ESC	=$1B		; escape
INMASK  =$7F
...
;IOBUF	=$01		; I/O buffer for source code input and analysis ; Don moved to 1 to avoid range errors
...
MISC2H	=$53	
KTEMP1	=$54		; general purpose storage
KTEMP2	=$55	
KTEMP3	=$56	
TEMP4	=$57
LMNE	=KTEMP3		; alias for compression and expansion routines
RMNE	=TEMP4
...
PRFLAG	=$5B
TEMP    = $60      ; temporary zero page address for calculations range errors DONALD
...
SHELL			; Loops forever
	...
	LDA #PROMPT
	JSR OUTCH	; prompt
	JSR KOUTSP	; can drop this if desperate for 3 more bytes :-)
@KEY	JSR GETCH
	CMP #BS
...
; ****************************************
; PANIC routine removed to prevent overwriting first byte of loaded file 
; at $2000 ($01 or ARGS+2). ; SHINIT call in PANIC cleared $2000, causing 
; data corruption after successful loads. ; Removal ensures U command 
; loads files correctly, even with erroneous input (e.g., U E). ; System 
; stability maintained as SHINIT is called elsewhere (e.g., boot). 
; PANIC: ; JSR SHINIT ; LDA ARGS+2 ; BNE @SKIP ; LDA #$01 ;@SKIP STA (SRCSTL),Y RTS
PANIC
	JSR SHINIT
	LDA ARGS+2
	BNE @SKIP
...
;@CLR STA IOBUF-1,X
@CLR STA <(IOBUF-1),X
;@CLR STA_IOBUF_MINUS1_XX 
	 DEX
	 BNE @CLR
	 RTS
;.define OFFSET_ADJUST 1
;
;FILLSP
;    LDA #SP
;FILBUF          ; fill the buffer with the contents of A
...
;	rts
FCH ; DONALD
	JMP SAVE_KRUSADER_SRC
	rts
UPL
	JMP KR_LOAD
	rts
CTG
	JMP LS_ASSEMBLY_SUBDIRNAME
	rts
ZAP
	JMP RM_KRUSADER
	rts
...
GETARG			; get the one or two numeric arguments
			...
			; store them in KTEMP1-4 as found
...
@DONE	LDY YSAV
	RTS		; m in KTEMP1,2, n in KTEMP3,4
...
GETSZ			; SIZE = KTEMP1,2 = lastlnL,H - MISCL,H + 1
	;LDX #-$04
	LDX #$FC  ; Don Range fix
@LOOP	LDA CURLNH+1,X
...
;	CPY #$00
	BEQ LCLERR
	STY YSAV
@DOIT	JSR GOTOLN	; this leaves KTEMP1 in Y and KTEMP2 in X
	CPX #FAIL
	BEQ LCLERR
	LDA CURLNL
	STA MISC2L
	...
	BEQ @INC
	LDX TEMP4
	LDY KTEMP3	; Validate the range arguments
	CPX KTEMP2	; First compare high bytes
	BNE @CHK	; If TEMP4 != KTEMP2, we just need to check carry
	CPY KTEMP1	; Compare low bytes when needed
...
TOEND	
	LDA #$FF
	STA KTEMP2	; makes illegal line number
;	JMP GOTOLN	; so CURLNL,H will be set to the end
; falls through

; ****************************************
	
GOTOLN			; go to line number given in KTEMP1,2
			; sets CURLNL,H to the appropriate address
			; and leaves KTEMP1 in Y and KTEMP2 in X
...
@NXTLN			; is the current line number the same 
			; as specified in KTEMP1,2?
			; Z set if equal
			; C set if KTEMP1,2 >= LINEL,H 
	LDY KTEMP1
	CPY LINEL
	BNE @NO
	LDX KTEMP2
...
@SKIP	CMP #SP		; valid ASCII range is $20 to $5D
	...
	CPY #BS
	BEQ KINPUT	; just do it all again
@NOBS	CPY #CR
	BNE LFAIL
...
INSSPC	
	...
	LDA <(IOBUF-1),X
	;LDA_IOBUF_MINUS1_X ; Use the macro to avoid the range error
	CMP #SP
	BEQ @JUMP
...
UPDTCL			; update the current line pointer 
	...
	STY KSCRTCH
	CLC
	ADC KSCRTCH	; move the current line pointer forward by 'Y' bytes
	STA CURLNL
	BCC INCLN
...
@CONT	CPY #$23	; STZ needs special handling
	...
	ADC #$F1    ; carry is set ; Don Range fix (use twos compliment of -15)
...
@MKARG			; where appropriate, the arg value is in LVALL,H
	...
	; word arg if X is greater than or equal to KABS
	CPX #KABS-1
	BMI DOBYTE	; X < #KABS	
DOWORD	JSR DOBYTE
	LDA LVALH
...
@LOOP	LDA (PTCHTL),Y	; copy symbol to COMM
	...
	INY 
	LDA (PTCHTL),Y
	STA KTEMP1	; save any offset value
...
CHKMOD	LDA CURMNE	; always > 0
	CMP KMIN,X	; mode index in X
	BCC BAD		; mnemonic < KMIN
...
@RET	RTS
@CONT	STA KTEMP1	; store the value of the byte in KTEMP1
	PLA
	CMP #PLUS
	BEQ @NONEG
	LDA KTEMP1
...
@NONEG	LDA HADFRF
	BEQ @SKIP
	LDA KTEMP1	; save the offset for use when patching
	STA (MISCL),Y	
@SKIP	;JMP ADD16X
; falls through
...
EVAL			; Evaluate an argument expression
	...
	STX KTEMP3	; store start of the expression
	LDA ARGS,X
	CMP #LOBYTE
	BEQ @HASOP
	CMP #HIBYTE
...
;
;	IMP (0)	ZPG (4) INZ (7) KABS (A) IND (D)
...
@DSMBL2	JSR INSTDSP	; Disassemble and display instr.
	...
	STY PCH
	DEC COUNT	; Done first 19 instrs	DON UN-COMMENT
	BNE @DSMBL2	; * Yes, loop.  Else DSMBL 20th	DON UN-COMMENT
	RTS			; DON ADD RTS

; DON COMMENT ENTIRE NEXT SECTION	
...
PRBL2	JSR KOUTSP	; Output a blank
	DEX   
...
@SKIP	DEX
	...
	LSR
	LSR
	STA KTEMP2	; save the high nibble
	LDA KTEMP1
	AND #$0F
	CMP #$08
	BNE @NOTC1
	LDA KTEMP2	; high nibble is our index
	RTS
@NOTC1	LDA KTEMP1
	AND #$1F
	CMP #$10
...
@DOC3	LDA KTEMP2
	...
	ADC #$10
	RTS
@NOTC3	LDX KTEMP1	; does this code end in 10010?
	TXA
	AND #$1F
	CMP #$12
	BNE @1
...
KMIN			; Minimum legal value for MNE for each mode.
	.byte $00, $30, $25, $39
	.byte $20, $28, $34
	.byte $28, $28, $28
	.byte $20, $28, $28
...
;NUMCMD	=$0D
;CMDS	.asciiz "NLXEMRDI!$AVP"
;NUMCMD	=$0F
;CMDS	.asciiz "NLXEMRDI!$AVPOF" ; MOD DONALD MORAN
NUMCMD	=$11
;CMDS	.asciiz "NLXEMRDI!$AVPFUCZ" ; MOD DONALD MORAN
CMDS	.asciiz "NLXEMRDI!$AVPSGCZ" ; MOD DONALD MORAN
...
V1 = VALUE-1
P1 = PANIC-1
;O1 = OPT-1 ; MOD DONALD MORAN

;F1 = FCH-1 ; FETCH   MOD DONALD MORAN
;U1 = UPL-1 ; UPLOAD  MOD DONALD MORAN
S1 = FCH-1 ; SAVE MOD DONALD MORAN
G1 = UPL-1 ; GET MOD DONALD MORAN

C1 = CTG-1 ; CATALOG MOD DONALD MORAN
Z1 = ZAP-1 ; ZAP     MOD DONALD MORAN
;C1 = CLG-1 ; MOD DONALD MORAN
...
CMDH	.byte	>N1
	...
	.byte	>V1
	.byte	>P1
;	.byte	>F1 ; MOD DONALD MORAN
;	.byte	>U1 ; MOD DONALD MORAN
	.byte	>S1 ; MOD DONALD MORAN
	.byte	>G1 ; MOD DONALD MORAN
	.byte	>C1 ; MOD DONALD MORAN
	.byte	>Z1 ; MOD DONALD MORAN
...
MSG     .asciiz "3.1 NESSEW NEK YB 20C56 REDASURK"
        .byte LF ; DONALD
		.byte   CR
...
@CONT				      ;  Line received, now let's parse it
				...
				LDY #$FF    ; Reset text index ; Don Range Error fix use 2's compliment
